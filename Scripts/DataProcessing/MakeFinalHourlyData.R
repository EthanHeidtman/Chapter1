...
# Description:    Loads each tidied file, renames columns, merges all together, 
#                 and then aggregates to hourly and saves to HourlyDataFinal.csv
# =============================================================================
...
inflows <- read.csv('Data/Tidied/Processed/MariettaDischarge.csv')
inflows <- inflows %>%
   dplyr::select(2, 3) %>%
...
discharge <- read.csv('Data/Tidied/Processed/ConowingoDischarge.csv')
discharge <- discharge %>%
   dplyr::select(2, 3) %>%
   mutate(DateTime = as_datetime(DateTime)) %>%                # Convert to datetime
...
salinity <- read.csv('Data/Tidied/Processed/DNRSalinityData.csv')
salinity <- salinity %>%
   dplyr::select(2, 3) %>%
   mutate(DateTime = as_datetime(DateTime))                    # Convert to datetime

# Tidal Data (and Fitted): MEASURED IN METERS ABOVE THE MEAN LOWER LOW WATER (MLLW) DATUM
tides <- read.csv('Data/Tidied/Processed/FittedTides.csv')
tides <- tides %>%
...
ferc$Date <- format(seq.Date(as.Date("2001-03-01"), 
                             as.Date("2002-02-28"), by = 1),
                    format = "%m-%d")
ferc <- ferc %>%
   rename(flow = V1) %>%
...
write.csv(data, 'Data/Tidied/Processed/HourlyDataFinal.csv')
...
ggsave('FERC.png', p1, path = 'Outputs/Plots', dpi = 600)
