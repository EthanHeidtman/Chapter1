...
run_one_experiment <- function(experiment_type, experiment_name, base_config) {
   ...
   result <- tryCatch({
      run_rolling_covariance_model(config)
   }, error = function(e) {
...
load_covariance_results <- function(experiment_type, base_path = "Outputs/Experiments/CovarianceModeling") {
...
unnest_covariance_results <- function(results_df, experiment_col = "experiment_name") {
