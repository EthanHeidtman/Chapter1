...
for(k in lead_times) {
   ...
   screened_data[[paste0("lag", k)]] <- read_qs_files(
      paste0('Data/Tidied/Final/Hourly/FinalDataScreened_lag', k, '.qs')
   )
   
   # Read RF results
   rf_results[[paste0("lag", k)]] <- read_qs_files(
      paste0('Outputs/Experiments/Models/HourlyRF/RFHourlyScreening_lag', k, '.qs')
   )
...
base_dir <- "Outputs/Plots/HourlyGAM"
...
plot_salinity_with_models(
   ...
   models = c('Prediction_lag168', 'Prediction_lag336'),
...
plot_salinity_with_models(
   data = all_data,
   date_range = c('2007-01-01', '2024-12-31'),
   models = c('Prediction_lag504'),
   title = "October 2016 High Salinity Event"
)
...
create_salinity_predictor_plot(
   ...
   models = c('Prediction_lag8', 'Prediction_lag10', 'Prediction_lag12'),
   predictors = c('RollingDischarge48', 'TideRange48', 'RollingV168'),
