...
p1 <- ggplot(group_importance_by_k, aes(x = LeadTime, y = RelativeImportance, fill = Group)) +
   geom_area(alpha = 0.7) +
...
for (k in lead_times) {
   lag_name <- paste0("lag", k)
   metrics_k <- rf_results[[lag_name]]$metrics
   metrics_k$LeadTime <- k
   all_metrics <- rbind(all_metrics, metrics_k)
}

# Plot RMSE across lead times
ggplot(all_metrics, aes(x = LeadTime, y = RMSE)) +
   geom_line(aes(group = 1), stat = "summary", fun = "median", linewidth = 1) +
   ...
   geom_ribbon(stat = "summary", 
               ...
               alpha = 0.2) +
   labs(
      title = "Model Performance Degradation Across Lead Times",
      x = "Lead Time (hours)",
      y = "RMSE",
      subtitle = "Line shows median, ribbon shows IQR across folds"
   ) +
   theme_minimal()

p2 <- ggplot(group_importance_by_k, aes(x = LeadTime, y = MeanImportance, color = Group)) +
