...
# Flushing threshold to match 01_CreateDailyPredictors.R
# Recompute here from the same source
raw_model_data <- as.data.frame(read_qs_files('Data/Tidied/Final/Daily/FinalModelData.qs'))
FLUSH_THRESHOLD <- quantile(raw_model_data$MaxDischarge[month(as.Date(raw_model_data$DateTime)) %in% c(8, 9, 10, 11)], 0.90, na.rm = TRUE)
...
# Load climatological baseline — must match 01_CreateDailyPredictors.R
# Computed from the full model data, not just the scenario year
...
# raw_df already filtered to YEAR — join climatology to get daily clim values
raw_df <- raw_df %>%
   mutate(DayOfYear = as.numeric(format(as.Date(DateTime), "%j"))) %>%
...
compute_discharge_preds <- function(q, var_names, clim_vals) {
   ...
   for (nm in var_names) {
      ...
      if (grepl("RollingDischarge", nm)) {
         ...
         # Anomaly = q / climatological mean for that day of year,
         # then rolling mean of that ratio — matches engineering script
         anomaly  <- q / clim_vals
...
for (LEAD_TIME in lead_times) {
   ...
   build_pred_df <- function(raw_df, q_modified) {
      disc_df <- compute_discharge_preds(q_modified, discharge_base, clim_vals = raw_df$ClimDischarge)
      base_df <- bind_cols(
