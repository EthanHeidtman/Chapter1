# =============================================================================
# Script Name:    RollingWindowModel.py
# Project:        Chapter1
# Author:         Ethan Heidtman
# Date Created:   2025-08-14
# Last Updated:   2025-08-14
# Description:    Implements rolling window distribution fitting (Phase 2) and
#                 window-based risk modeling (Phase 3) for salinity exceedance
...
class RollingWindowModel:
    ...
    def fit_rolling_distributions(self):
        ...
        # window_days = int(model.config.window_length)
        threshold = float(self.config.salinity_threshold)
        # threshold = float(model.config.salinity_threshold)
        min_obs = getattr(self.config, 'min_observations_per_window', 10)
        # min_obs = getattr(model.config, 'min_observations_per_window', 10)
    ...
    def fit_window_model(self):
        """Phase 3: Fit rolling window model in logit space"""
        print("Phase 3: Fitting rolling window model...")
        
        # Align distribution results with predictor data
        dist_times = pd.to_datetime(self.rolling_distributions['timestamp'])
        # dist_times = pd.to_datetime(model.rolling_distributions['timestamp'])
        dist_times_index = pd.Index(dist_times) # make dist_times an index
        pred_times = self.predictor_data.index
        # pred_times = model.predictor_data.index
        common_times = dist_times_index.intersection(pred_times)
        
        if len(common_times) < 20:
            raise ValueError("Insufficient overlapping time periods for window modeling")
        
        # Prepare aligned data
        dist_aligned = self.rolling_distributions.set_index('timestamp').loc[common_times]
        # dist_aligned = model.rolling_distributions.set_index('timestamp').loc[common_times]
        pred_aligned = self.predictor_data.loc[common_times, self.predictors]
        # pred_aligned = model.predictor_data.loc[common_times, model.predictors]
        ...
        # y_logit = model.logit_transform(y_probs)
        ...
        print(f"Using {len(y_logit)} valid observations for window modeling")
        
        # Fit rolling window models
        window = int(self.config.window_length)
        # window = int(model.config.window_length)
        use_shrinkage = getattr(self.config, 'use_shrinkage', False)
        # use_shrinkage = getattr(model.config, 'use_shrinkage', False)
        
        window_results = []

        
        for i in range(window - 1, len(valid_times)):
        ...
        if not window_results:
            raise ValueError("No successful window model fits!")
            
        self.window_model = pd.DataFrame(window_results)
        print(f"Fitted window models for {len(self.window_model)} time periods")
        
        return self.window_model
    ...
    def rolling_distributions_with_ci(self, confidence_levels=[0.50, 0.75, 0.90, 0.95]):
        ...
        """
        if self.rolling_distributions is None or self.window_model is None:
            raise ValueError("Must fit rolling distributions and window model first!")
    
        # Merge rolling distributions with window model on timestamp
    ...
    def predict_future_exceedance(self, future_data, confidence_levels=[0.50, 0.75, 0.90, 0.95]):
        ...
        """
        if self.window_model is None:
            raise ValueError("window model must be fitted first.")
    
        results = []
        latest_model = self.window_model.iloc[-1]  # Use most recent fitted coefficients
    ...
    def fit_complete_model(self):
        ...
        print("=== ROLLING WINDOW MODEL FOR SALINITY MANAGEMENT ===")
        ...
        # model.fit_rolling_distributions()
        ...
        # model.prepare_predictor_data()
        print(f"After predictor prep: {len(self.predictor_data)} rows, NaN count: {self.predictor_data.isna().sum().sum()}")
        
        # Step 4: Fit window model (Phase 3)
        self.fit_window_model()
        # model.fit_window_model()
        print(f"After window model: {len(self.window_model)} rows, NaN count: {self.window_model.isna().sum().sum()}")
        ...
        print(f"  - window model periods: {len(self.window_model)}")
...
def run_rolling_model(config, confidence_levels=[0.50, 0.75, 0.90, 0.95]):
    ...
    try:
        ...
        model = RollingWindowModel(config)
        ...
        results = {
            ...
            'summary': {
                ...
                'window_periods': len(model.window_model),
            }
        }
