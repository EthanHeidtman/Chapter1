...
plot_salinity_forecast_panels <- function(data,
   ...
   # ---------------------------------------------------------------------
   # Expects long-format data with columns:
   #   DateTime    - issue date t (when predictors were measured)
   #   h           - forecast horizon, in days
   #   Salinity_h  - observed salinity at the target date (t + h)
   #   Predicted   - model forecast for that (DateTime, h) row
   #
   # Each panel = one horizon. Both observed and predicted are plotted
   # against the TARGET date (DateTime + h), so a day's observed value
   # lines up with the forecast that was issued h days earlier for it.
   # This is what makes "the h=14 line shows what was predicted 14 days
   # in advance for this day" a correct reading of the x-axis.
   # ---------------------------------------------------------------------
   
   # ---- Styling Parameters ----
   observed_linewidth <- 0.9
   model_linewidth    <- 1.3   # thickened so model lines stand out against observed
   observed_alpha     <- 0.8
   model_alpha        <- 1.0
   ...
   if (is.null(horizons)) {
      stop("horizons must be specified, e.g. horizons = c(1, 5, 10, 15, 20)")
   }
   
   # ---- Build target-date column and filter to requested horizons ----
   data <- data %>%
      dplyr::mutate(TargetDate = DateTime + lubridate::days(h)) %>%
      dplyr::filter(h %in% horizons)
   
   # ---- Filter to date window, evaluated on TargetDate ----
   if (!is.null(year)) {
   ...
   # ---- Helper: build one panel for a given horizon ----
   ...
   # ---- Build panels (one per horizon, ordered as given) ----
